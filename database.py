...
def select_top_of_karma():
    with engine.connect() as conn:
        req = text('SELECT "username", "karma" FROM public.supports ORDER BY "karma" DESC LIMIT 5;')
        res = conn.execute(req)
        res = res.all()
        if res is None:
            return []
...
def insert_new_support(username: str, user_id: int):
    try:
        with engine.connect() as conn:
            req = text('SELECT * FROM public.supports WHERE "username"=:username;').bindparams(username=username)
            res = conn.execute(req)
            if not res.first():
                req = text('INSERT INTO public.supports ("username", "user_id", "karma") VALUES (:user_name, :user_id, :karma);')
                req = req.bindparams(user_name=username, user_id=user_id, karma=0)
                conn.execute(req)
                conn.commit()
                return ""
            return "User is does exist!"
    except Exception as e:
        return e
