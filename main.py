...
def save(user_data, telegram_id, username, request_text):
    the_daterange = user_data["date"].split(" - ")
...
async def send_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("✅ Принять", callback_data=f"accept_{bd_id}_{user.id}_{user.username}")],
    ]
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    sup_id, rate = "0", "0"
    if query.data.split("_")[0] == "rate":
        action, bd_id, rate, sup_id = query.data.split('_')
        leader_id = 0
        username = ""
    elif query.data.split("_")[0] == "finish":
        action, bd_id, leader_id, username, sup_id = query.data.split('_')
    else:
        action, bd_id, leader_id, username = query.data.split('_')
    ...
    if action == "rate":
        ...
        if select_request_status(int(bd_id)) != 0:
            ...
            keyboard = [
                [InlineKeyboardButton("🟢 5", callback_data=f"rate_{bd_id}_5_{sup_id}"),
                InlineKeyboardButton("🟡 4", callback_data=f"rate_{bd_id}_4_{sup_id}"),
                InlineKeyboardButton("🟠 3", callback_data=f"rate_{bd_id}_3_{sup_id}"),
                InlineKeyboardButton("🟤 2", callback_data=f"rate_{bd_id}_2_{sup_id}"),
                InlineKeyboardButton("🔴 1", callback_data=f"rate_{bd_id}_1_{sup_id}")]
            ]
            await context.bot.send_message(
                chat_id=leader_id,
                text=f"Пожалуйста, оцените помощь Дежурного!",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
    elif action == "retry":
        if select_request_status(int(bd_id)) != 3:
            ...
            keyboard = [
                [InlineKeyboardButton("✅ Принять", callback_data=f"accept_{bd_id}_{query.from_user.id}_{query.from_user.username}")],
            ]
            await context.bot.send_message(
                chat_id=Settings.CHAT_ID,
                text=message,
        ...
        keyboard = [
            [InlineKeyboardButton("✅ Да", callback_data=f"finish_{bd_id}_{leader_id}_{username}_{query.from_user.id}"),
             InlineKeyboardButton("❌ Нет", callback_data=f"retry_{bd_id}_{leader_id}_{username}")]
        ]
        await context.bot.send_message(
            chat_id=leader_id,
        ...
        if stat == 0:
            ...
            keyboard = [
                [InlineKeyboardButton("✅ Да", callback_data=f"submit_{bd_id}_{leader_id}_{username}"),
                 InlineKeyboardButton("❌ Нет", callback_data=f"denied_{bd_id}_{leader_id}_{username}")]
            ]
            await context.bot.send_message(
                chat_id=leader_id,
